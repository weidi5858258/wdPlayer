...
void init_opts(void) {
    LOGI("cmdutils_av_dict_set()\n");
    av_dict_set(&sws_dict, "flags", "bicubic", 0);
}

void uninit_opts(void) {
    LOGI("cmdutils_uninit_opts()\n");
...
void show_help_children(const AVClass *pClass, int flags) {
    ...
    if (pClass->option) {
        av_opt_show2(&pClass, NULL, flags, 0);
        printf("\n");
    }

    while (child = av_opt_child_class_next(pClass, child))
...
void uninit_parse_context(OptionParseContext *octx) {
    LOGI("cmdutils_uninit_parse_context()\n");
    int i, j;
...
int split_commandline(OptionParseContext *octx, int argc, char *argv[],
                      const OptionDef *options,
                      const OptionGroupDef *groups, int nb_groups) {
    LOGI("cmdutils_split_commandline()\n");
    int optindex = 1;
    int dashdash = -2;
...
AVDictionary **setup_find_stream_info_opts(AVFormatContext *s,
    ...
    if (!opts) {
        LOGE("cmdutils_setup_find_stream_info_opts() Could not alloc memory for stream options.\n");
        return NULL;
    }
    for (i = 0; i < s->nb_streams; i++)
        opts[i] = filter_codec_opts(
                codec_opts, s->streams[i]->codecpar->codec_id, s, s->streams[i], NULL);
    return opts;
}
