...
static AVInputFormat *file_iformat = nullptr;
...
static bool test_delay_one_time = false;
...
static int frame_queue_nb_remaining(FrameQueue *f) {
    return f->size - f->rindex_shown;// 相当于: f->size - 1;
}
...
static void video_refresh(void *opaque, double *remaining_time) {
    ...
    if (is->useMediaCodec) {
        ...
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            *remaining_time = 0.0;
            ...
            if (frame_timer_delay - time > AV_SYNC_THRESHOLD_MAX) {
                LOGD("video_refresh()----------------------------------------------\n");
                LOGE("video_refresh()              is->frame_timer = %lf\n", is->frame_timer);
                LOGE("video_refresh()                        delay = %lf\n", delay);
        ...
        if (video_refresh_log) {
            LOGI("video_refresh()              *remaining_time = %.7lf\n", *remaining_time);
        }
...
static int configure_video_filters(AVFilterGraph *graph,
                                   ...
                                   const char *vfilters,
                                   AVFrame *frame) {
    LOGI("configure_video_filters() start\n");
    ...
    // video_size=1920x1080:pix_fmt=0:time_base=1/120000:pixel_aspect=1/1:frame_rate=30000/1001
    // video_size=3840x2160:pix_fmt=0:time_base=1/25000:pixel_aspect=1/1:frame_rate=25/1
    ...
    if (!filt) {
        LOGI("configure_video_filters() filt 1 is nullptr\n");
    }
    ret = avfilter_graph_create_filter(
            &filt_src, filt, "ffplay_buffer", buffersrc_args, nullptr, graph);
    if (ret < 0) {
        LOGE("configure_video_filters() avfilter_graph_create_filter 1 failed\n");
        goto fail;
    }

    filt = avfilter_get_by_name("buffersink");
    if (!filt) {
        LOGI("configure_video_filters() filt 2 is nullptr\n");
    }
    ret = avfilter_graph_create_filter(
            &filt_out, filt, "ffplay_buffersink", nullptr, nullptr, graph);
    if (ret < 0) {
        LOGE("configure_video_filters() avfilter_graph_create_filter 2 failed\n");
        goto fail;
    }

    ret = av_opt_set_int_list(
            filt_out, "pix_fmts", pix_fmts, AV_PIX_FMT_NONE, AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
        LOGE("configure_video_filters() av_opt_set_int_list failed\n");
        goto fail;
    }
...
#define INSERT_FILT(name, arg) do {                                          \
    ...
    if (ret < 0) {
        LOGE("configure_video_filters() configure_filtergraph failed\n");
        goto fail;
    }
    ...
    LOGI("configure_video_filters() end\n");
    fail:
    return ret;
}
...
static void *video_thread_mc(void *arg) {
    ...
    while (1) {
        ...
        if (pkt.data == flush_pkt.data) {
            LOGD("video_thread_mc() pkt.data == flush_pkt.data\n");
            avcodec_flush_buffers(d->avctx);
            d->finished = 0;
            d->next_pts = d->start_pts;
            d->next_pts_tb = d->start_pts_tb;

            pthread_mutex_lock(&pictq->pmutex);
            for (int i = 0; i < pictq->max_size; i++) {
                Frame *vp = &pictq->queue[i];
                frame_queue_unref_item(vp);
            }
            pictq->size = 0;
            pictq->rindex = 0;
...
int decoder_decode_frame_by_mediacodec(int roomIndex,
    ...
    frame->pkt_pts = pts_;
    frame->pkt_dts = dts_;//
    frame->best_effort_timestamp = pts_;
    frame->pkt_size = size;//
    frame->pkt_pos = pos_;//
    frame->pkt_duration = duration_;//
    ...
    if (last_format != frame->format
        ...
        LOGI("decoder_decode_frame_by_mediacodec() Video frame changed from\n"
             ...
             " to\n"
             " size:%d x %d format:%s serial:%d vfilter_idx:%d\n",
             last_w, last_h,
             (const char *) av_x_if_null(av_get_pix_fmt_name(last_format), "none"),
             last_serial, last_vfilter_idx,
             frame->width, frame->height,
             (const char *) av_x_if_null(
                     av_get_pix_fmt_name(static_cast<AVPixelFormat>(frame->format)), "none"),
             is->viddec.pkt_serial, is->vfilter_idx);
        LOGI("decoder_decode_frame_by_mediacodec() avfilter_graph_free\n");
        avfilter_graph_free(&graph);
        LOGI("decoder_decode_frame_by_mediacodec() avfilter_graph_alloc\n");
        graph = avfilter_graph_alloc();
        if (!graph) {
            ret = AVERROR(ENOMEM);
        ...
        graph->nb_threads = filter_nbthreads;
        if (!vfilters_list) {
            LOGI("decoder_decode_frame_by_mediacodec() vfilters_list is nullptr\n");
        }
        if ((ret = configure_video_filters(graph, is,
                                           vfilters_list
                                           ? vfilters_list[is->vfilter_idx]
...
static int stream_component_open(VideoState *is, int stream_index) {
    ...
    if (forced_codec_name) {
        LOGI("stream_component_open() forced_codec_name = %s\n", forced_codec_name);
...
static int create_avformat_context(void *arg) {
    ...
    if (avFormatContext != nullptr) {
        LOGI("create_avformat_context() avFormatContext isn't nullptr\n");
        avformat_free_context(avFormatContext);
        avFormatContext = nullptr;
    }
    LOGI("create_avformat_context() avformat_alloc_context\n");
    avFormatContext = avformat_alloc_context();
    if (avFormatContext == nullptr) {
        LOGE("create_avformat_context() Could not allocate context.\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    is->ic = avFormatContext;
    avFormatContext->interrupt_callback.callback = decode_interrupt_cb;// 设置回调函数
    avFormatContext->interrupt_callback.opaque = is;// 回调函数的参数
    ...
    if (seek_by_bytes < 0) {
        int flag1 = avFormatContext->iformat->flags & AVFMT_TS_DISCONT;
        int flag2 = strcmp("ogg", avFormatContext->iformat->name);
    ...
    if (!window_title && (t = av_dict_get(avFormatContext->metadata, "title", nullptr, 0))) {
    ...
    for (int i = 0; i < avFormatContext->nb_streams; i++) {
        ...
        if (type >= 0 && wanted_stream_spec[type] && st_index[type] == -1) {
            if (avformat_match_stream_specifier(avFormatContext, st, wanted_stream_spec[type]) >
                0) {
                st_index[type] = i;
            }
        }
    ...
    // 得到媒体文件的总时长
    media_duration = (long) (avFormatContext->duration / AV_TIME_BASE);
    LOGI("create_avformat_context() media_duration = %ld\n", media_duration);
    if (avFormatContext->duration != AV_NOPTS_VALUE) {
        // 得到的是秒数
        media_duration = (long) ((avFormatContext->duration + 5000) / AV_TIME_BASE);
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        AVRational sar = av_guess_sample_aspect_ratio(avFormatContext, st, nullptr);
        if (codecpar->width) {
...
static int stream_open(const char *filename, AVInputFormat *iformat) {
    ...
    if ((ret = create_avformat_context(is)) < 0) {
        LOGE("stream_open() create_avformat_context(is) < 0\n");
        goto fail;
    }
...
static void *video_play(void *arg) {
    ...
    if (is->useMediaCodec) {
        ...
        } else if (isLive && bit_rate == 0 && bit_rate_video == 0 && frame_rate >= 45) {
            // live 爱经典
        ...
        LOGD("video_play() test_remaining_time: %.10lf\n", test_remaining_time);
        /*if (isLocal) {
            if (frame_rate >= 45) {// 60
                test_remaining_time = 0.0;
