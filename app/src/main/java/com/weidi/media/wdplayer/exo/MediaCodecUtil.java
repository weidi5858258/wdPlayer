...
public final class MediaCodecUtil {
  ...
  static SpeedShiftRange.SpeedRange getSpeedShiftRange(String name, final boolean isVideo
    , final ExoMediaFormat exoMediaFormat) {
    Log.d(TAG, " : codec=" + name + ", mimeType=" + exoMediaFormat.mimeType);
    SpeedShiftRange.SpeedRange speedRange;
    speedRange = SpeedRangeCapabilities.getRange(name, exoMediaFormat.mimeType, isVideo,
            exoMediaFormat.sampleRate, exoMediaFormat.channelCount, exoMediaFormat.framerate,
            exoMediaFormat.bitrate, 0);
    if (speedRange == null) {
      Log.w(TAG, "not valid. return default range.");
      speedRange = SpeedRangeCapabilities.getDefaultRange(isVideo);
    }
    return speedRange;
  }

  static SpeedShiftRange.SpeedRange getDefaultSpeedShiftRange(final boolean isVideo) {
    return SpeedRangeCapabilities.getDefaultRange(isVideo);
  }
