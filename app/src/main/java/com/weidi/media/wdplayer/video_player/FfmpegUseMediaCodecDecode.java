...
public class FfmpegUseMediaCodecDecode {
    ...
    public void destroy() {
        ...
        if (mVideoInputDatasQueue != null) {
            mVideoInputDatasQueue.needToWait = false;
        }
        if (mVideoDatasIndexQueue != null) {
            mVideoDatasIndexQueue.needToWait = false;
        }
        if (mAudioInputDatasQueue != null) {
            mAudioInputDatasQueue.needToWait = false;
        }
        clearQueue();
        signalQueue();
            ...
            if (mVideoDatasIndexQueue != null
                ...
                try {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (object != null && object instanceof Integer) {
                            roomIndex = (int) object;
                            Log.i(TAG, "clearIndexQueue() roomIndex: " + roomIndex);
                            mVideoWrapper.decoderMediaCodec.releaseOutputBuffer(
                                    roomIndex, true);
                        }
                    }
    ...
    public boolean initAudioMediaCodec(JniObject jniObject) {
        ...
        mAudioInputDatasQueue = new ArrayBlockingQueue<AVPacket>(5);
        mAudioInputDatasQueue.needToWait = true;
    ...
    public boolean initVideoMediaCodec(JniObject jniObject) {
        ...
        if (width >= 3840 && height >= 2160 && frame_rate >= 45) {
            is4K = true;
        } else {
            is4K = false;
        }
            ...
            if (mVideoWrapper != null
                ...
                if (videoSerial != avPacket.serial) {
                    ...
                    if (mVideoWrapper.decoderMediaCodec != null
                            && videoSerial > 2
                            && is4K
                            /*&& ((mVideoWrapper.width >= 3840 && mVideoWrapper.height >= 2160) ||
                            mVideoWrapper.frameRate >= 45)*/) {
    ...
    public void videoDecodeSync() {
        Log.i(TAG, "syncVideoDecode() start");
        MediaCodec codec = mVideoWrapper.decoderMediaCodec;
        AVPacket avPacket = null;
        while (mVideoWrapper != null && mVideoWrapper.isHandling) {
            try {
                // 没有元素就会阻塞
                if (mVideoInputDatasQueue != null) {
                ...
                Log.e(TAG, "videoDecodeSync() " + e.toString());
                avPacket = null;
            }
            if (avPacket == null) {
                Log.e(TAG, "videoDecodeSync() return for avPacket is null");
                break;
            }
            ...
            if (!is4K) {
                if (!EDMediaCodec.feedInputBufferAndDrainOutputBuffer(
                        mCallback,
                        EDMediaCodec.TYPE.TYPE_VIDEO,
                        codec,
                        null,
    ...
    private long[] videoValueLongArray = new long[4];
    // audio
    ...
    private long[] audioValueLongArray = new long[4];
    ...
    private EDMediaCodec.Callback mCallback = new EDMediaCodec.Callback() {
        ...
        public void handleAudioOutputFormat(MediaFormat mediaFormat) {
            ...
            if (mExoAudioTrack.mAudioTrack == null) {
                ...
                mAudioWrapper.isHandling = false;
                handleAudioOutputBuffer(-1, null, null);
            }
        ...
        public int handleAudioOutputBuffer(int roomIndex, ByteBuffer room,
            ...
            audioValueLongArray[0] = 0;
            audioValueLongArray[1] = 0;
            audioValueLongArray[2] = 0;
            audioValueLongArray[3] = 0;
            mAudioJniObject.valueLongArray = audioValueLongArray;
    ...
    public final static boolean VIDEO_NEED_TO_ASYNC = true;
    private final static boolean AUDIO_NEED_TO_ASYNC = true;

    private ArrayBlockingQueue<AVPacket> mVideoInputDatasQueue = null;
    //new ArrayBlockingQueue<AVPacket>(5);
    private ArrayBlockingQueue<Integer> mVideoDatasIndexQueue = null;
    //new ArrayBlockingQueue<Integer>(5);
    private ArrayBlockingQueue<AVPacket> mAudioInputDatasQueue = null;
    //new ArrayBlockingQueue<AVPacket>(5);
    ...
    private MediaCodec.Callback mVideoAsyncDecoderCallback = new MediaCodec.Callback() {
        ...
        public void onInputBufferAvailable(@NonNull MediaCodec codec, int roomIndex) {
            ...
            if (avPacket == null) {
                Log.e(TAG, "onInputBufferAvailable() return for avPacket is null");
                try {
        ...
        public void onOutputBufferAvailable(@NonNull MediaCodec codec, int roomIndex,
            ...
            //Log.i(TAG, "onOutputBufferAvailable() roomIndex: " + roomIndex);
        ...
        public void onError(@NonNull MediaCodec mediaCodec, @NonNull MediaCodec.CodecException e) {
            ...
            mCallback.handleVideoOutputBuffer(-1, null, null);
        }
    ...
    private MediaCodec.Callback mAudioAsyncDecoderCallback = new MediaCodec.Callback() {
        ...
        public void onOutputBufferAvailable(@NonNull MediaCodec codec, int roomIndex,
            ...
            if (room != null) {
                room.position(roomInfo.offset);
                room.limit(roomInfo.offset + roomSize);
                mCallback.handleAudioOutputBuffer(roomIndex, room, roomInfo);
                room.clear();
            } else {
                mCallback.handleAudioOutputBuffer(roomIndex, null, null);
            }
        ...
        public void onError(@NonNull MediaCodec mediaCodec, @NonNull MediaCodec.CodecException e) {
            ...
            mCallback.handleAudioOutputBuffer(-1, null, null);
        }
